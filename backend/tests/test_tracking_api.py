...
class TrackingApiTests(unittest.TestCase):
    ...
    def test_workflow_acceptance_capture_generate_review_approve_and_audit_export(self) -> None:
        status, _ = self._request_json(
            "PUT",
            "/api/v1/profile",
            {
        ...
        application_id = self._capture_application(company="Nimbus", title="Platform Engineer")
        ...
        self.assertTrue(resume_version_id)
        ...
        self.assertIn("change_log", body)
        self.assertIn("claims_map", body)
        self.assertGreater(len(body["claims_map"]), 0)
        ...
        self.assertEqual(status, 200)
        self.assertEqual(body["application"]["id"], application_id)
        self.assertEqual(body["job_posting"]["application_id"], application_id)
        self.assertEqual(len(body["resume_versions"]), 1)
        self.assertEqual(body["resume_versions"][0]["id"], resume_version_id)
